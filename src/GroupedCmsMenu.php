...
class GroupedCmsMenu extends Extension
{
    ...
    {
		Requirements::css('resources/vendor/symbiote/silverstripe-grouped-cms-menu/css/GroupedCmsMenu.css');
    }
                ...
                foreach ($menuItems as $key => $menuItem) {
                    if (is_numeric($key)) {
                        $itemsToGroup[$menuItem] = array(
                            'Group' => $groupName,
                            'Priority' => (array_key_exists('priority',
                                $groupSettings[$groupName])) ? $groupSettings[$groupName]['priority'] : $groupSort,
                            'SortOrder' => $itemSort
                        );
                    }
                    $itemSort++;
        ...
        foreach (GroupedList::create($items->sort(array('Priority' => 'DESC')))->groupBy('Group') as $group => $children) {
            if (count($children) > 1) {
                ...
                $result->push(ArrayData::create(array(
                    ...
                    'Children' => $config->get('LeftAndMain',
                        'menu_groups_alphabetical_sorting') ? $children->sort('Title') : $children->sort('SortOrder')
                )));
            } else {
